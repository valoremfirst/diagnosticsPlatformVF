...
import { listCompanies, listSessions } from "@/lib/store";
...
export default async function AnalyticsPage() {
    ...
    return (
      <div className="animate-fade-in">
        <PageHeader
          ...
          description="Trends, benchmarks and risk signals across every diagnostic you've run."
        />
        <EmptyState
          icon={BarChart3}
          title="No analytics yet"
          description="Analyse at least one transcript and the portfolio charts will populate here automatically."
          action={
            <Link href="/">
  ...
  const companyRanking = [...byCompany.entries()]
    .map(([key, scores]) => {
      const company = companies.find((c) => c.id === key);
      const name = company?.name ?? scored.find((s) => (s.companyId ?? s.companyName) === key)?.companyName ?? key;
      return {
        name,
        score: avg(scores),
    ...
    <div className="animate-fade-in">
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          value={scored.length}
          hint={`across ${byCompany.size} companies`}
          accent="ink"
        />
        <MetricCard
