...
export async function GET(
  ...
  return NextResponse.json({ sessions: listSectionSessions(params.id, fn) });
}
...
export async function POST(
  ...
  // Accept either pre-structured turns or raw pasted/uploaded text.
  ...
  if (typeof body.transcript === "string") {
    ...
    return NextResponse.json(
      { error: "transcript must be pasted text or an array of turns." },
      { status: 400 },
    );
  }
  ...
  const session = createSectionSession(params.id, fn, {
    title,
    transcript,
    status: "processing",
  ...
  try {
    ...
    const updated = setResult(session.id, result);
